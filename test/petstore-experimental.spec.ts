...
const gen = new NgOpenApiGen(petstore as OpenAPIObject, options);
gen.generate();
...
describe('Generation tests using petstore-experimental.json', () => {
  ...
  it.skip('listPets-fn properties', done => {
    const pets = gen.services.get('pets');
  ...
  it.skip('createPets-fn properties', done => {
    const pets = gen.services.get('pets');
  ...
  it.skip('showPetById-fn properties', done => {
    const pets = gen.services.get('pets');
